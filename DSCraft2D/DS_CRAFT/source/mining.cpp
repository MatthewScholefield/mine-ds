...
int non_placeable[]={STICK,WOOD_PICK,COAL,WOOD_SHOVEL,COBBLE_PICKAXE,LAPIS,COBBLE_SHOVEL,COBBLE_AXE,IRON,IRON_PICKAXE,DIAMOND,GOLD,REDSTONE};//List of items you can not place...
#define NON_PLACEABLE_NUM 13
int mine_frame;
int mine_timeout;
u16* donegfx;
int olax, olay;
...
void drawBlock(int block,int x,int y){
	...
	switch(block)
	{
		...
		case IRON_PICKAXE: IRON_PICKAXE_render(x,y); break;
		case WOOD_SHOVEL: WOOD_SHOVEL_render(x,y); break;
		case WOOD_AXE: WOOD_AXE_render(x,y); break;
		case COAL: COAL_render(x,y); break;
		case IRON: IRON_render(x,y); break;
		case DOOR: DOOR_render(x,y); break;
		case DIAMOND: DIAMOND_render(x,y); break;
		case REDSTONE: REDSTONE_render(x,y); break;
		case GOLD: GOLD_render(x,y); break;
